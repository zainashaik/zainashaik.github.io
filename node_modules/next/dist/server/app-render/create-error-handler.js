...
function createFlightReactServerErrorHandler(dev, onReactServerRenderError) {
    return (thrownValue)=>{
        ...
        if ((0, _bailouttocsr.isBailoutToCSRError)(thrownValue)) return thrownValue.digest;
        // If this is a navigation error, we don't need to log the error.
        if ((0, _isnextroutererror.isNextRouterError)(thrownValue)) return thrownValue.digest;
        // If this error occurs, we know that we should be stopping the static
        // render. This is only thrown in static generation when PPR is not enabled,
        // which causes the whole page to be marked as dynamic. We don't need to
        // tell the user about this error, as it's not actionable.
        if ((0, _hooksservercontext.isDynamicServerError)(thrownValue)) return thrownValue.digest;
