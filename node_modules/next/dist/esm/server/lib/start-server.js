...
let startServerSpan;
export async function getRequestHandlers({ dir, port, isDev, onCleanup, server, hostname, minimalMode, keepAliveTimeout, experimentalHttpsServer, quiet }) {
    return initialize({
        dir,
        port,
        hostname,
...
export async function startServer(serverOptions) {
    ...
    process.title = `next-server (v${"15.0.3"})`;
    let handlersReady = ()=>{};
    let handlersError = ()=>{};
    ...
    await new Promise((resolve)=>{
        server.on('listening', async ()=>{
            ...
            const networkUrl = hostname ? `${selfSignedCertificate ? 'https' : 'http'}://${actualHostname}:${port}` : null;
