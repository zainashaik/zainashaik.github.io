...
export async function collectSegmentData(fullPageDataBuffer, staleTime, clientModules, serverConsumerManifest) {
    ...
    const segmentTasks = [];
    const treeStream = await renderToReadableStream(// RootTreePrefetch is not a valid return type for a React component, but
...
async function PrefetchTreeData({ fullPageDataBuffer, serverConsumerManifest, clientModules, staleTime, segmentTasks }) {
    ...
    const initialRSCPayload = await createFromReadableStream(streamFromBuffer(fullPageDataBuffer), {
        serverConsumerManifest
    });
...
async function collectSegmentDataImpl(route, seedData, fullPageDataBuffer, clientModules, serverConsumerManifest, segmentPathStr, accessToken, segmentTasks) {
    ...
    waitAtLeastOneReactRenderTask().then(()=>renderSegmentPrefetch(seedData, segmentPathStr, accessToken, childAccessTokens, clientModules)));
...
async function renderSegmentPrefetch(seedData, segmentPathStr, accessToken, childAccessTokens, clientModules) {
    ...
    const segmentStream = await renderToReadableStream(segmentPrefetch, clientModules, {
