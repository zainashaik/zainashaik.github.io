...
async function createTreeCodeFromPath(pagePath, { page, resolveDir, resolver, resolveParallelSegments, metadataResolver, pageExtensions, basePath, buildInfo, flyingShuttle, collectedDeclarations }) {
          ...
          ${createMetadataExportsCode(metadata)}
            ...
            const definedFilePaths = filePaths.filter(([, filePath])=>{
                if (filePath !== undefined) {
                    var _buildInfo_route;
                    if (flyingShuttle && ((_buildInfo_route = buildInfo.route) == null ? void 0 : _buildInfo_route.relatedModules)) {
...
const nextAppLoader = async function nextAppLoader() {
    ...
    const { name, appDir, appPaths, pagePath, pageExtensions, rootDir, tsconfigPath, isDev, nextConfigOutput, preferredRegion, basePath, flyingShuttle, middlewareConfig: middlewareConfigBase64, nextConfigExperimentalUseEarlyImport } = loaderOptions;
