...
import { STATIC_STATUS_PAGE_GET_INITIAL_PROPS_ERROR, PUBLIC_DIR_MIDDLEWARE_CONFLICT, MIDDLEWARE_FILENAME, PAGES_DIR_ALIAS, INSTRUMENTATION_HOOK_FILENAME, RSC_PREFETCH_SUFFIX, RSC_SUFFIX, NEXT_RESUME_HEADER, PRERENDER_REVALIDATE_HEADER, PRERENDER_REVALIDATE_ONLY_GENERATED_HEADER, NEXT_CACHE_REVALIDATE_TAG_TOKEN_HEADER, NEXT_CACHE_REVALIDATED_TAGS_HEADER, MATCHED_PATH_HEADER } from '../lib/constants';
...
import { createPagesMapping, getPageFromPath, getStaticInfoIncludingLayouts, sortByPageExts } from './entries';
...
import { ACTION_HEADER, NEXT_ROUTER_PREFETCH_HEADER, RSC_HEADER, RSC_CONTENT_TYPE_HEADER, NEXT_ROUTER_STATE_TREE_HEADER, NEXT_DID_POSTPONE_HEADER, NEXT_ROUTER_SEGMENT_PREFETCH_HEADER } from '../client/components/app-router-headers';
...
import { recordFrameworkVersion, updateBuildDiagnostics, recordFetchMetrics, updateIncrementalBuildMetrics } from '../diagnostics/build-diagnostics';
import { getStartServerInfo, logStartInfo } from '../server/lib/app-info-log';
import { hasCustomExportOutput } from '../export/utils';
import { getTurbopackJsConfig, handleEntrypoints, handleRouteType, handlePagesErrorRoute, formatIssue, isRelevantWarning, isPersistentCachingEnabled } from '../server/dev/turbopack-utils';
...
import { detectChangedEntries } from './flying-shuttle/detect-changed-entries';
        ...
        await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            if (config.experimental.clientRouterFilter) {
                ...
                if (isFullFlyingShuttle) {
                    ...
                    ].map((entry)=>normalizeAppPath(getPageFromPath(entry, config.pageExtensions))), ...((changedPagePathsResult == null ? void 0 : changedPagePathsResult.unchanged.pages.length) ? ((_changedPagePathsResult_changed = changedPagePathsResult.changed) == null ? void 0 : _changedPagePathsResult_changed.pages) || [] : []).map((item)=>getPageFromPath(item, config.pageExtensions)));
                } else {
                    filterPaths.push(...appPaths);
                }
                clientRouterFilters = createClientRouterFilter(filterPaths, config.experimental.clientRouterFilterRedirects ? nonInternalRedirects : [], config.experimental.clientRouterFilterAllowedRate);
            ...
            await recordFrameworkVersion("15.0.3");
            ...
            if (!isCompileMode && (combinedPages.length > 0 || useStaticPages404 || useDefaultStatic500 || appDir)) {
                ...
                await staticGenerationSpan.traceAsyncFn(async ()=>{
                    ...
                    staticPaths.forEach((prerenderedRoutes, originalAppPath)=>{
                        ...
                        unknownPrerenderRoutes = getSortedRouteObjects(unknownPrerenderRoutes, (prerenderedRoute)=>prerenderedRoute.path);
                        knownPrerenderRoutes = getSortedRouteObjects(knownPrerenderRoutes, (prerenderedRoute)=>prerenderedRoute.path);
                        prerenderedRoutes = [
                        ...
                        if (!hasRevalidateZero && isDynamicRoute(page)) {
                            ...
                            for (const route of dynamicRoutes){
                                ...
                                const fallbackMode = fallbackModes.get(originalAppPath) ?? FallbackMode.NOT_FOUND;
                                ...
                                let fallbackRevalidate = isRoutePPREnabled && fallbackMode === FallbackMode.PRERENDER ? revalidate ?? false : undefined;
                                const fallback = fallbackModeToFallbackField(fallbackMode, route);
                                ...
                                prerenderManifest.dynamicRoutes[route] = {
                                    ...
                                    renderingMode: isAppPPREnabled ? isRoutePPREnabled ? RenderingMode.PARTIALLY_STATIC : RenderingMode.STATIC : undefined,
                                    experimentalBypassFor: bypassFor,
                                    routeRegex: normalizeRouteRegex(getNamedRouteRegex(route, false).re.source),
                                    dataRoute,
                                    fallback,
                                    fallbackRevalidate,
                                    fallbackStatus: meta.status,
                                    fallbackHeaders: meta.headers,
                                    dataRouteRegex: !dataRoute ? null : normalizeRouteRegex(getNamedRouteRegex(dataRoute.replace(/\.rsc$/, ''), false).re.source.replace(/\(\?:\\\/\)\?\$$/, '\\.rsc$')),
                                    prefetchDataRoute,
                                    prefetchDataRouteRegex: !prefetchDataRoute ? undefined : normalizeRouteRegex(getNamedRouteRegex(prefetchDataRoute.replace(/\.prefetch\.rsc$/, ''), false).re.source.replace(/\(\?:\\\/\)\?\$$/, '\\.prefetch\\.rsc$')),
                                    allowHeader: ALLOWED_HEADERS
                                };
                            }
                        }
                            ...
                            if (!isDynamic) {
                                ...
                                for (const route of additionalPaths.get(page) ?? []){
                                    ...
                                    prerenderManifest.routes[route.path] = {
                                        ...
                                        renderingMode: undefined,
                                        srcRoute: page,
                                        dataRoute: path.posix.join('/_next/data', buildId, `${normalizePagePath(route.path)}.json`),
                                        // Pages does not have a prefetch data route.
                                        prefetchDataRoute: undefined,
                                        allowHeader: ALLOWED_HEADERS
                                    };
