...
export async function getSourceMapFromCompilation(id, compilation) {
    try {
...
export async function getSource(filename, options) {
    ...
    if (filename.startsWith("" + assetPrefix + "/_next/static")) {
        filename = path.join(distDirectory, filename.replace(new RegExp("^" + assetPrefix + "/_next/"), ''));
    }
