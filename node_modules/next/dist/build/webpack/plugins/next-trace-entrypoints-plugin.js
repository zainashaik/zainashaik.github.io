...
const _nextswcloader = /*#__PURE__*/ _interop_require_default(require("../loaders/next-swc-loader"));
...
class TraceEntryPointsPlugin {
    constructor({ rootDir, appDir, pagesDir, compilerType, optOutBundlingPackages, appDirEnabled, traceIgnores, esmExternals, outputFileTracingRoot, flyingShuttle, swcLoaderConfig }){
    ...
    async createTraceAssets(compilation, assets, span) {
        ...
        await span.traceChild('create-trace-assets').traceAsyncFn(async ()=>{
            ...
            const distDir = _path.default.join(outputPath, this.compilerType === 'server' ? '../..' : '../');
    ...
    tapfinishModules(compilation, traceEntrypointsPluginSpan, doResolve, readlink, stat) {
        async function getOriginalHash(path) {
            return new Promise((resolve, reject)=>{
                if (path.includes('?')) {
            ...
            await finishModulesSpan.traceAsyncFn(async ()=>{
                        ...
                        if (isApp || isPage) {
                            for (const dep of entry.dependencies){
                                ...
                                if (entryMod && entryMod.resource === '') {
                                    ...
                                    if (moduleBuildInfo.route) {
                                        ...
                                        if (this.pagesDir && absolutePath.startsWith(this.pagesDir) || this.appDir && absolutePath.startsWith(this.appDir)) {
                                            var _moduleBuildInfo_route;
                                            if (this.flyingShuttle) {
                                                this.traceHashes.set(absolutePath, await getOriginalHash(absolutePath));
                                            }
                                            ...
                                            if (this.flyingShuttle && ((_moduleBuildInfo_route = moduleBuildInfo.route) == null ? void 0 : _moduleBuildInfo_route.relatedModules)) {
                                                var _moduleBuildInfo_route1;
                                                let curAdditionalEntries = additionalEntries.get(name);
                                                if (!curAdditionalEntries) {
                                                    curAdditionalEntries = new Map();
                                                    additionalEntries.set(name, curAdditionalEntries);
                                                }
                                                for (const item of (_moduleBuildInfo_route1 = moduleBuildInfo.route) == null ? void 0 : _moduleBuildInfo_route1.relatedModules){
                                ...
                                if (entryMod && entryMod.resource) {
                                    ...
                                    if (this.flyingShuttle) {
                                        this.traceHashes.set(entryMod.resource, await getOriginalHash(entryMod.resource));
                                    }
                ...
                const readFile = async (path)=>{
                    ...
                    if (this.flyingShuttle) {
                        ...
                        if (!source || (0, _utils.isClientComponentEntryModule)(mod)) {
                        ...
                        if (usingOriginalSource && // don't attempt transpiling CSS or image imports
                    ...
                    for (const dep of mod.dependencies){
                        ...
                        if ((depMod == null ? void 0 : depMod.resource) && !depModMap.get(depMod.resource)) {
                            if (this.flyingShuttle) {
                                ...
                                this.traceHashes.set(depMod.resource, await getOriginalHash(depMod.resource));
                            }
                    ...
                    for (const entry of entryPaths){
                        ...
                        if (this.flyingShuttle) {
                            ...
                            for (const item of curDepSet || []){
                                ...
                                if (!item.includes('?')) {
                                    var _parentFilesMap_get2;
                                    if (!finalDeps.has(item)) {
                                        finalDeps.set(item, {
                                            bundled: true
                                        });
                                    }
                                    const parentFiles = ((_parentFilesMap_get2 = parentFilesMap.get(_path.default.relative(this.tracingRoot, entry))) == null ? void 0 : _parentFilesMap_get2.entries()) || [];
