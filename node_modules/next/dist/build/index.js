...
const _detectchangedentries = require("./flying-shuttle/detect-changed-entries");
const _storeshuttle = require("./flying-shuttle/store-shuttle");
const _stitchbuilds = require("./flying-shuttle/stitch-builds");
const _inlinestaticenv = require("./flying-shuttle/inline-static-env");
...
async function build(dir, reactProductionProfiling = false, debugOutput = false, runLint = true, noMangling = false, appDirOnly = false, turboNextBuild = false, experimentalBuildMode, traceUploadUrl) {
    ...
    try {
        ...
        await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            const isStoreOnlyFlyingShuttle = (flyingShuttle == null ? void 0 : flyingShuttle.mode) === 'store-only';
            const shuttleDir = _path.default.join(distDir, 'cache', 'shuttle');
            if (flyingShuttle) {
                await _fs.promises.mkdir(shuttleDir, {
                    recursive: true
                });
            ...
            if (pagesPaths && isFullFlyingShuttle) {
                await nextBuildSpan.traceChild('detect-changed-entries', {
                    type: 'pages'
                }).traceAsyncFn(async ()=>{
                    changedPagePathsResult = await (0, _detectchangedentries.detectChangedEntries)({
                        pagesPaths,
            ...
            if (appDir) {
                ...
                if (appPaths && isFullFlyingShuttle) {
                    await nextBuildSpan.traceChild('detect-changed-entries', {
                        ...
                        changedAppPathsResult = await (0, _detectchangedentries.detectChangedEntries)({
                            appPaths,
                            pageExtensions: config.pageExtensions,
                            distDir,
                            shuttleDir,
            ...
            if (config.experimental.clientRouterFilter) {
                ...
                if (isFullFlyingShuttle) {
                    var _changedAppPathsResult_unchanged, _changedAppPathsResult_changed, _changedPagePathsResult_changed;
                    filterPaths.push(...[
                        // client filter always has all app paths
                        ...(changedAppPathsResult == null ? void 0 : (_changedAppPathsResult_unchanged = changedAppPathsResult.unchanged) == null ? void 0 : _changedAppPathsResult_unchanged.app) || [],
                        ...(changedAppPathsResult == null ? void 0 : (_changedAppPathsResult_changed = changedAppPathsResult.changed) == null ? void 0 : _changedAppPathsResult_changed.app) || []
                    ].map((entry)=>(0, _apppaths.normalizeAppPath)((0, _entries.getPageFromPath)(entry, config.pageExtensions))), ...((changedPagePathsResult == null ? void 0 : changedPagePathsResult.unchanged.pages.length) ? ((_changedPagePathsResult_changed = changedPagePathsResult.changed) == null ? void 0 : _changedPagePathsResult_changed.pages) || [] : []).map((item)=>(0, _entries.getPageFromPath)(item, config.pageExtensions)));
                } else {
                    filterPaths.push(...appPaths);
                }
                clientRouterFilters = (0, _createclientrouterfilter.createClientRouterFilter)(filterPaths, config.experimental.clientRouterFilterRedirects ? nonInternalRedirects : [], config.experimental.clientRouterFilterAllowedRate);
            ...
            await (0, _builddiagnostics.recordFrameworkVersion)("15.0.3");
            ...
            if (!isGenerateMode) {
                if (turboNextBuild) {
                    const { duration: compilerDuration, shutdownPromise: p, ...rest } = await turbopackBuild();
                    shutdownPromise = p;
            ...
            if (isFullFlyingShuttle) {
                ...
                if (await (0, _fileexists.fileExists)(_path.default.join(shuttleDir, 'server'))) {
                    await (0, _recursivecopy.recursiveCopy)(_path.default.join(shuttleDir, 'server'), _path.default.join(distDir, 'server'), {
                        filter (item) {
                            // we copy page chunks separately to not copy stale entries
                            return !item.startsWith('/middleware.js') && !item.match(/^[/\\](pages|app)[/\\]/);
                        },
                        overwrite: false
                    });
                }
                                    ...
                                    if (pageType === 'app' && originalAppPath) {
                                        ...
                                        if ((0, _isedgeruntime.isEdgeRuntime)(pageRuntime)) {
                                            ...
                                            if (workerResult.prerenderedRoutes && workerResult.prerenderedRoutes.length > 0) {
                                                ...
                                                ssgPageRoutes = workerResult.prerenderedRoutes.map((route)=>route.path);
                                                isSSG = true;
                                            }
            ...
            if (!isGenerateMode) {
                if (flyingShuttle) {
                    ...
                    if (isStoreOnlyFlyingShuttle) {
                        ...
                        await nextBuildSpan.traceChild('stitch-incremental-builds').traceAsyncFn(async ()=>{
                            const stitchResult = await (0, _stitchbuilds.stitchBuilds)({
                                config,
                                buildId,
                                distDir,
                                shuttleDir,
                                ...
                                encryptionKey,
                                allowedErrorRate: config.experimental.clientRouterFilterAllowedRate
                            }, {
                                changed: {
                                    pages: (changedPagePathsResult == null ? void 0 : changedPagePathsResult.changed.pages) || [],
                                    app: (changedAppPathsResult == null ? void 0 : changedAppPathsResult.changed.app) || []
                                },
                                unchanged: {
                                    pages: (changedPagePathsResult == null ? void 0 : changedPagePathsResult.unchanged.pages) || [],
                                    app: (changedAppPathsResult == null ? void 0 : changedAppPathsResult.unchanged.app) || []
                                },
                                pageExtensions: config.pageExtensions
                            });
                    ...
                    await nextBuildSpan.traceChild('store-cache-shuttle').traceAsyncFn(async ()=>{
                        await (0, _storeshuttle.storeShuttle)({
                            config,
                            distDir,
                            shuttleDir
                        });
            ...
            if (!isCompileMode && (combinedPages.length > 0 || useStaticPages404 || useDefaultStatic500 || appDir)) {
                ...
                await staticGenerationSpan.traceAsyncFn(async ()=>{
                    ...
                    staticPaths.forEach((prerenderedRoutes, originalAppPath)=>{
                        ...
                        const appConfig = appDefaultConfigs.get(originalAppPath) || {};
                        let hasRevalidateZero = appConfig.revalidate === 0 || ((_exportResult_byPath_get = exportResult.byPath.get(page)) == null ? void 0 : _exportResult_byPath_get.revalidate) === 0;
                        ...
                        unknownPrerenderRoutes = (0, _utils.getSortedRouteObjects)(unknownPrerenderRoutes, (prerenderedRoute)=>prerenderedRoute.path);
                        knownPrerenderRoutes = (0, _utils.getSortedRouteObjects)(knownPrerenderRoutes, (prerenderedRoute)=>prerenderedRoute.path);
                        prerenderedRoutes = [
                        ...
                        for (const prerenderedRoute of prerenderedRoutes){
                            ...
                            if (prerenderedRoute.path === _constants1.UNDERSCORE_NOT_FOUND_ROUTE) {
                                continue;
                            }
                        ...
                        for (const route of routes){
                            if ((0, _utils.isDynamicRoute)(page) && route === page) continue;
                            if (route === _constants1.UNDERSCORE_NOT_FOUND_ROUTE) continue;
                        ...
                        if (!hasRevalidateZero && (0, _utils.isDynamicRoute)(page)) {
                            ...
                            for (const route of dynamicRoutes){
                                const normalizedRoute = (0, _normalizepagepath.normalizePagePath)(route);
                                const { metadata, revalidate } = exportResult.byPath.get(route) ?? {};
                                let dataRoute = null;
                                if (!isAppRouteHandler) {
                                    dataRoute = _path.default.posix.join(`${normalizedRoute}${_constants.RSC_SUFFIX}`);
                                }
                                ...
                                const fallbackMode = fallbackModes.get(originalAppPath) ?? _fallback.FallbackMode.NOT_FOUND;
                                ...
                                let fallbackRevalidate = isRoutePPREnabled && fallbackMode === _fallback.FallbackMode.PRERENDER ? revalidate ?? false : undefined;
                                const fallback = (0, _fallback.fallbackModeToFallbackField)(fallbackMode, route);
                                ...
                                prerenderManifest.dynamicRoutes[route] = {
                                    ...
                                    renderingMode: isAppPPREnabled ? isRoutePPREnabled ? _renderingmode.RenderingMode.PARTIALLY_STATIC : _renderingmode.RenderingMode.STATIC : undefined,
                                    experimentalBypassFor: bypassFor,
                                    routeRegex: (0, _loadcustomroutes.normalizeRouteRegex)((0, _routeregex.getNamedRouteRegex)(route, false).re.source),
                                    dataRoute,
                                    fallback,
                                    fallbackRevalidate,
                                    fallbackStatus: meta.status,
                                    fallbackHeaders: meta.headers,
                                    dataRouteRegex: !dataRoute ? null : (0, _loadcustomroutes.normalizeRouteRegex)((0, _routeregex.getNamedRouteRegex)(dataRoute.replace(/\.rsc$/, ''), false).re.source.replace(/\(\?:\\\/\)\?\$$/, '\\.rsc$')),
                                    prefetchDataRoute,
                                    prefetchDataRouteRegex: !prefetchDataRoute ? undefined : (0, _loadcustomroutes.normalizeRouteRegex)((0, _routeregex.getNamedRouteRegex)(prefetchDataRoute.replace(/\.prefetch\.rsc$/, ''), false).re.source.replace(/\(\?:\\\/\)\?\$$/, '\\.prefetch\\.rsc$')),
                                    allowHeader: ALLOWED_HEADERS
                                };
                            }
                        }
                        ...
                        if (isSsg) {
                            ...
                            if (!isDynamic) {
                                ...
                                if (i18n) {
                                    ...
                                    for (const locale of i18n.locales){
                                        ...
                                        prerenderManifest.routes[localePage] = {
                                            initialRevalidateSeconds: ((_exportResult_byPath_get = exportResult.byPath.get(localePage)) == null ? void 0 : _exportResult_byPath_get.revalidate) ?? false,
                                    ...
                                    prerenderManifest.routes[page] = {
                                        initialRevalidateSeconds: ((_exportResult_byPath_get1 = exportResult.byPath.get(page)) == null ? void 0 : _exportResult_byPath_get1.revalidate) ?? false,
                                        experimentalPPR: undefined,
                                        renderingMode: undefined,
                                        srcRoute: null,
                                        dataRoute: _path.default.posix.join('/_next/data', buildId, `${file}.json`),
                                ...
                                if (pageInfo) {
                                    ...
                                    pageInfo.initialRevalidateSeconds = ((_exportResult_byPath_get2 = exportResult.byPath.get(page)) == null ? void 0 : _exportResult_byPath_get2.revalidate) ?? false;
                                }
                            } else {
                                ...
                                for (const route of additionalPaths.get(page) ?? []){
                                    ...
                                    const pageFile = (0, _normalizepagepath.normalizePagePath)(route.path);
                                    await moveExportedPage(page, route.path, pageFile, isSsg, 'html', true);
                                    await moveExportedPage(page, route.path, pageFile, isSsg, 'json', true);
                                    if (hasAmp) {
                                    ...
                                    const initialRevalidateSeconds = ((_exportResult_byPath_get3 = exportResult.byPath.get(route.path)) == null ? void 0 : _exportResult_byPath_get3.revalidate) ?? false;
                                    ...
                                    prerenderManifest.routes[route.path] = {
                                        ...
                                        renderingMode: undefined,
                                        srcRoute: page,
                                        dataRoute: _path.default.posix.join('/_next/data', buildId, `${(0, _normalizepagepath.normalizePagePath)(route.path)}.json`),
                                        // Pages does not have a prefetch data route.
                                        prefetchDataRoute: undefined,
                                        allowHeader: ALLOWED_HEADERS
                                    };
