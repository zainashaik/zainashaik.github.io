...
See more info here: https://nextjs.org/docs/messages/gsp-redirect-during-prerender`);data.props={__N_REDIRECT:data.redirect.destination,__N_REDIRECT_STATUS:getRedirectStatus(data.redirect)},void 0!==data.redirect.basePath&&(data.props.__N_REDIRECT_BASE_PATH=data.redirect.basePath),metadata.isRedirect=!0}if((dev||isBuildTimeSSG)&&!metadata.isNotFound&&!isSerializableProps(pathname,"getStaticProps",data.props))throw Error("invariant: getStaticProps did not return valid props. Please report this.");if("revalidate"in data){if(data.revalidate&&"export"===renderOpts.nextConfigOutput)throw Error('ISR cannot be used with "output: export". See more info here: https://nextjs.org/docs/advanced-features/static-html-export');if("number"==typeof data.revalidate){if(Number.isInteger(data.revalidate)){if(data.revalidate<=0)throw Error(`A page's revalidate option can not be less than or equal to zero for ${req.url}. A revalidate option of zero means to revalidate after _every_ request, and implies stale data cannot be tolerated.
